...
```{r}
represa <- read_sf("dados/represa/SIRGAS_SHP_represanivelmax.shp") %>% st_set_crs("epsg:31983")


r <- raster::rasterize(x = result %>% select(x, y), y = raster,
                       field = result %>%
                         select(populacao))
result.raster <- raster_to_matrix(r)


result.raster %>% 
  sphere_shade(texture = "imhof1") %>%
  plot_3d(result.raster, zscale = 1000, triangulate = T)
...
plot_gg(gg, multicore = TRUE, scale = 150, width = 5, height = 7, pointcontract = 1, 
        theta = -50, phi = 40, zoom = .65, fov = 16, windowsize = c(3840, 2160))
