...
df <- IPTU.censo %>% 
  ...
  summarize(
            ...
            verticalizacao = weighted.mean(pavimentos, area_ocupada * percent_intersec)) %>% 
  
  mutate(densidade_terreno = populacao / area_terreno,
         densidade_setor = populacao / area_setor,
         cota_parte_IPTU = area_terreno / unidades,
...
# cor(area_setor, area_terreno)
# lm(densidade_setor ~ CA + cota_parte_censo + verticalizacao + exp(-distancia_centro), 
#                  data = df %>% filter(densidade_terreno<300000)) %>% 
#   lm.beta() %>% 
#   summary()
...
lm(populacao ~ area_construida, data = df) %>% summary()

df %>%
  pivot_longer(c(densidade_setor, densidade_terreno)) %>% 
  filter(value < 300000) %>% 
  ggplot(aes(x = distancia_centro, y = value)) +
  geom_point() +
...
```{r}
# modelos = list(
#   "Simples" = lm(densidade_setor ~ CA + cota_parte + verticalizacao, 
#                  data = df),
#   "Com Distância" = lm(densidade_setor ~ CA + cota_parte + verticalizacao + exp(-distancia_centro), 
#                        data = df),
#   "Simples_rm" = lm(densidade_setor ~ CA + cota_parte + verticalizacao, 
#                     data = df %>% filter(cota_parte < 1000)),
#   "Com Distância_rm" = lm(densidade_setor ~ CA + cota_parte + verticalizacao + exp(-distancia_centro), 
#                           data = df %>% filter(cota_parte < 1000))
# )
# 
# modelsummary::modelsummary(modelos, statistic = NULL, estimate = "{estimate} {stars}")
```



```{r}
# lm(densidade_setor ~ CA + cota_parte + verticalizacao, data = df %>% filter(cota_parte < 1000)) %>% summary()
# lm(densidade_terreno ~ CA + cota_parte + verticalizacao, data = df) %>% summary()
# 
...
# 
# lm(densidade_setor ~ exp(-distancia_centro) + CA + cota_parte + verticalizacao, 
