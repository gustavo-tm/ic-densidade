...
ggsave("tex/imagens/indicadores_tempo.png", gg, width = 8, height = 9, dpi = 300)
gg
```
...
df %>% 
  ...
  filter(domicilios > 0) %>% 
  mutate(domicilios_irregulares = domicilios - unidades,
         taxa_irregular = domicilios_irregulares / domicilios,
         populacao_irregular = populacao * taxa_irregular,
         area_irregular = area_setor * taxa_irregular) %>% 
  summarize(populacao = sum(populacao),
            populacao_irregular = sum(populacao_irregular, na.rm = TRUE),
            area_total = sum(area_setor),
            area_irregular = sum(area_irregular, na.rm = TRUE)) %>% 
  mutate(taxa_pop_irregular = populacao_irregular / populacao,
         taxa_area_irregular = area_irregular / area_total) %>% 
  pivot_longer(everything())
```
```{r}
df %>% 
  ...
  bind_rows(censo %>% 
              ...
              anti_join(df %>% select(id_setor_censitario))) %>% 
  filter(populacao > 0) %>% 
  mutate(taxa_irregular = (domicilios - unidades) / domicilios) %>% 
  arrange(desc(populacao/area_setor)) %>% 
  pivot_longer(c(populacao, area_setor, domicilios), names_to = "variavel", values_to = "total") %>% 
  mutate(irregular = total * taxa_irregular,
         regular = total - irregular) %>% 
  group_by(variavel) %>% 
  mutate(irregular_cum = cumsum(irregular),
         regular_cum = cumsum(regular),
         setor = row_number()) %>% 
  ggplot() +
  geom_line(aes(x = setor, y = regular_cum, color = "Regular")) +
  geom_line(aes(x = setor, y = irregular_cum, color = "Irregular")) +
...
```{r write_regression1, echo = FALSE}
# Escrever a tabela em um documento .tex
write(modelsummary::modelsummary(list("Todos setores" = modelo1, 
                                      "Balanço entre 25% e 75%" = modelo2, 
                                      "Balanço entre 40% e 60%" = modelo3), 
                                 gof_omit = "RMSE", estimate = "{estimate} {stars}", 
                                 statistic = "({std.error})", 
                                 output = "latex", 
                                 caption = "Tabela de regressão (metodologia join)",
                                 label = "tab:reg1"), 
      file="tex/tabelas/reg_join.tex")

```

```{r}
modelsummary::modelsummary(list("Todos setores" = modelo1, 
                                "Até 50% de irregularidade" = modelo2,
                                "Até 20% de irregularidade" = modelo3), 
                                 gof_omit = "RMSE", estimate = "{estimate} {stars}", 
                                 statistic = "({std.error})",
                                 output = "gt") %>% 
  tab_spanner(label = "Variável dependente: densidade populacional", columns = 2:4) %>% 
  tab_footnote("Espectro de irregularidade é definido como unidades / (unidades + domicílios)") %>% 
  gtsave("tex/tabelas/regressao.tex", as_latex = TRUE)
```
...
ggsave("tex/imagens/importancia.png", gg, width = 7, height = 4, dpi = 300)
...
modelo2 <- df %>% 
  filter(abs(espectro_irregularidade - .5) < .1) %>% 
  lm(densidade ~ cota_parte + verticalizacao + CA + area_setor, data = .) %>% 
  summary()

modelo3 <- df %>% 
  filter(abs(espectro_irregularidade - .5) < .1) %>% 
  mutate(area_setor_quad = area_setor^2) %>% 
  lm(densidade ~ cota_parte + verticalizacao + CA + area_setor + area_setor_quad, data = .) %>% 
  summary()

resultado <- modelsummary::modelsummary(
...
modelsummary::modelsummary(
  list("Todos setores" = modelo1, "Até 50% de irregularidade" = modelo2, "Até 20% de irregularidade" = modelo3), 
  gof_omit = "RMSE", estimate = "{estimate} {stars}", 
  statistic = NULL)
```


```{r analise_tree_rastar, cache = TRUE}
...
#                      data = train, num.trees = 1000, importance = "permutation")
...
```
```{r}
rf_model_rest <- ranger::ranger(densidade ~ ., 
                     data = train %>% select(-c(populacao, domicilios)), num.trees = 5000, importance = "permutation")
rf_preds_rest <- predict(rf_model_rest, data = test)$predictions

ggplot(NULL, aes(x = rf_model_rest$variable.importance,
                 y = fct_reorder(names(rf_model_rest$variable.importance),
                                 rf_model_rest$variable.importance))) +
    geom_col(fill = "darkblue", alpha = 0.75) +
    labs(x = "Predictor", y = "Importance", title = "Random Forest") +
    theme_classic()
```
...
```{r analise_result}
ggplot(data = tibble(x = rf_preds_all, y = test$densidade, espectro_irregularidade = test$espectro_irregularidade)) +
  geom_point(aes(x = x, y = y, color = espectro_irregularidade), alpha = .5, size = 2.5) +
  scale_color_gradient2(high = "blue", low = "red", mid = "grey", midpoint = .5) +
