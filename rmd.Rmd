...
```{r}
df %>% 
  select(id_setor_censitario, populacao, domicilios, unidades) %>% 
  bind_rows(censo %>% 
              select(id_setor_censitario, populacao = v0001, domicilios = v0002) %>% 
              mutate(unidades = 0) %>% 
              anti_join(df %>% select(id_setor_censitario))) %>% 
  mutate(pop_por_domicilio = populacao / domicilios,
         domicilios_irregulares = domicilios - unidades,
         populacao_irregular = pop_por_domicilio * domicilios_irregulares) %>% 
  summarize(populacao = sum(populacao),
            populacao_irregular = sum(populacao_irregular, na.rm = TRUE)) %>% 
  mutate(percent_irregular = populacao_irregular / populacao)
```
...
modelsummary::modelsummary(list("Todos setores" = modelo1, 
  ...
  tab_footnote("Balanço é definido como unidades / (unidades + domicílios)") %>% 
...
```{r analise_result}
ggplot(data = tibble(x = rf_preds_all, y = test$densidade, balanco = test$balanco)) +
  geom_point(aes(x = x, y = y, color = balanco), alpha = .5, size = 2.5) +
