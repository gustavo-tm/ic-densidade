...
gg <- IPTU.lotes %>% 
  ...
  filter(cota_parte < 600, CA < 5, pavimentos < 15) %>% 
...
```{r disparidade_unidades, cache = TRUE}
gg <- IPTU.censo %>% 
  group_by(id_setor_censitario) %>% 
  summarize(domicilios = median(v0002),
            unidades = sum(unidades)) %>% 
  
  # Caso seja 50%, domicilios = unidades, 
  # caso < 50%, mais domicílios no censo que unidades no IPTU, 
  # caso > 50%, há mais unidades no IPTU do que domicílios no censo
...
temp <- censo.pontos %>%
  ...
  mutate(intersects = st_intersects(
    samples, 
    st_union(favela %>% st_union() %>% st_buffer(1))) %>% 
      as.character(),
    intersects = intersects == "1")

temp %>% 
  group_by(intersects, erro) %>% 
  summarize(n = n())

(temp %>% 
  ggplot() +
  geom_sf(data = distrito, color = NA) +
  geom_sf(data = st_union(favela %>% st_union() %>% st_buffer(1)),
          aes(fill = "Favelas e lotes irregulares"), 
          color = "black", size = .1, alpha = .7) +
  geom_sf(aes(geometry = samples, color = intersects), alpha = .25, size = .2) +
  scale_fill_manual("", values = c("Favelas e lotes irregulares" = "#2BAA92FF")) +
  scale_color_manual(values = c("TRUE" = "black", "FALSE" = "green")) +
  theme_void()) %>% 
  ggsave("test.png", ., height = 20, width = 20, dpi = 300)
```

```{r}
temp.dist <- censo.pontos %>% 
  st_set_geometry("samples") %>% 
  st_set_crs("epsg:31983") %>% 
  mutate(distancia_favela = st_distance(samples,st_union(favela %>% st_union() %>% st_buffer(1))) %>% 
           as.numeric()) %>% 
  st_drop_geometry()

temp.dist %>% 
  ggplot() +
  geom_violin(aes(x = "", y = distancia_favela, fill = erro)) +
  scale_y_sqrt()
```
...
ggplot() +
  ...
  geom_sf(data = df %>%
            mutate(decil = ntile(densidade, 10)),
...
```{r}
n <- length(test$densidade)

resultados <- tibble(pred = c(rf_preds, rf_preds_all, lm_preds),
                     real = c(test$densidade, test$densidade, test$densidade),
                     modelo = c(rep("A", n), rep("B", n), rep("C", n)))

ggplot(data = resultados) +
  geom_point(aes(x = pred, y = real)) +
  theme(aspect.ratio = 1) +
