...
geoms.censo <- censo %>% 
  mutate(area = st_area(geometry) %>% as.numeric()) %>% 
  select(id = id_setor_censitario, area, populacao = v0001)
...
result.censo <- exact_extract(raster, geoms.censo, include_xy = T, 
                        include_cols = c("id", "area", "populacao"), 
                        force_df = T, coverage_area = T, progress = F) %>% 
  bind_rows %>% 
  mutate(percent_intersect = coverage_area / area,
         across(c(area, populacao), ~ .x * percent_intersect)) %>% 
  group_by(x, y) %>% 
...
ggsave("tex/imagens/rasters.png", gg, width = 20, height = 20, dpi = 400)
