...
gg <- IPTU %>% 
  ...
  select("Cota Parte" = cota_parte, 
         "Coeficiente de Aproveitamento (CA)" = CA, 
...
gg <- IPTU %>% 
  ...
  labs(fill = "Número de ocorrências", colour = "", x = "Verticalização",
       y = "Coeficiente de Aproveitamento (CA)") +
  theme(aspect.ratio = 1)
...
gg <- ggcorrplot::ggcorrplot(result |> 
                     ...
                     select(tx_ocupacao, verticalizacao, cota_parte, CA, densidade_total, densidade_residencial) |> 
                     drop_na() |> 
                     cor(),
...
split <- rsample::initial_split(result %>% 
                                  ...
                                  filter(abs(espectro_irregularidade - .5) <.1,
                                         area > max(area) * .99) %>% 
                                  select(-c(densidade_total, populacao, area, cota_parte_inversa)) |> 
                                  mutate(placebo = rnorm(length(densidade_residencial))),
                                prop = 0.7)
train <- rsample::training(split)
test <- rsample::testing(split)
...
rtree <- rpart::rpart(densidade_residencial ~ CA + cota_parte + verticalizacao, data = train)
...
split <- rsample::initial_split(result %>% 
                                  ...
                                  filter(abs(espectro_irregularidade - .5) <.1,
                                         area > max(area) * .99) %>% 
                                  mutate(placebo = rnorm(length(densidade_residencial))) %>%
                                  select(-c(densidade_residencial, populacao, area, cota_parte_inversa)), 
                                prop = 0.7)
train <- rsample::training(split)
test <- rsample::testing(split)

rf_model <- ranger::ranger(densidade_total ~ ., 
                     data = train, num.trees = 50000, importance = "permutation")
rf_preds <- predict(rf_model, data = test)$predictions
